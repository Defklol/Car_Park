...
static uint32_t mpu9250_write_burst(uint8_t reg, uint8_t * p_data, uint32_t length)
{
    uint32_t err_code;
...
uint32_t mpu9250_write_single(uint8_t reg, uint8_t data)
{
    uint32_t err_code;
    uint32_t timeout = MPU9250_TWI_TIMEOUT;
...
uint32_t ak8963_write_single(uint8_t reg, uint8_t data)
{
    uint32_t err_code;
    uint32_t timeout = MPU9250_TWI_TIMEOUT;
    ...
    err_code = nrf_drv_twi_tx(m_twi_instance, MPU9250_REG_AK8963_ASTC, packet, 2, false);
    if(err_code != NRF_SUCCESS) return err_code;
...
uint32_t read_registers(uint8_t halname, uint8_t * p_data, uint32_t length)
{
    uint32_t err_code;
    uint32_t timeout = MPU9250_TWI_TIMEOUT;
        
    err_code = nrf_drv_twi_rx(m_twi_instance, halname, p_data, length);
    if(err_code != NRF_SUCCESS) return err_code;
...
uint32_t ak8963_read_registers(uint8_t reg, uint8_t * p_data, uint32_t length)
{
    uint32_t err_code;
    uint32_t timeout = MPU9250_TWI_TIMEOUT;
    
    err_code = nrf_drv_twi_tx(m_twi_instance, MPU9250_REG_AK8963_ASTC, &reg, 1, false);
    if(err_code != NRF_SUCCESS) return err_code;
    ...
    err_code = nrf_drv_twi_rx(m_twi_instance, MPU9250_REG_AK8963_ASTC, p_data, length);
    if(err_code != NRF_SUCCESS) return err_code;
...
uint32_t mpu9250_read_mag(mag_values_t * mag_values)
{
    ...
    if(err_code != NRF_SUCCESS) return err_code;
		mpu9250_write_single(MPU9250_REG_AK8963_CNTL, 0x01);
...
uint32_t read_mag()
{
	ak8963_write_single(MPU9250_REG_AK8963_ST1, 0x01);
}
*/
